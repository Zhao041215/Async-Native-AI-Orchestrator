"""V8 Typed Domain Models — PostgreSQL-backed, contract-validated kernel."""
...
def normalize_database_url(url: str) -> str:
    # psycopg uses postgresql:// (no +psycopg prefix); strip SQLAlchemy prefix if present
...
class ScaleProfileData(StrictModel):
    name: str = "medium"
    label: str = ""
    kernel_generation: str = "v8_pg_native"
    mission_contract_version: str = "8.0"
...
class RequirementsResult(LooseModel):
    status: str = "ok"
    summary: str = ""
    goals: list[str] = Field(default_factory=list)
    users: list[str | dict[str, Any]] = Field(default_factory=list)
...
class ArchitectureResult(LooseModel):
    architecture_summary: Any = ""
    technology_choices: Any = Field(default_factory=list)
    ...
    design_principles: Any = Field(default_factory=list)
    primary_risks: Any = Field(default_factory=list)
    validation_commands: Any = Field(default_factory=list)


class PackageDAG(LooseModel):
    packages: list[dict[str, Any]] = Field(default_factory=list)
    waves: list[dict[str, Any]] = Field(default_factory=list)
    edges: list[dict[str, str]] = Field(default_factory=list)


class RunMetadata(LooseModel):
    """LooseModel (extra=ignore) so sub-phase cache keys don't break validation."""
    ...
    # Sub-phase retry cache: stores intermediate AI results keyed by _SEED_KEY etc.
...
class AISchedulerLimits(StrictModel):
    ...
    scheduler_max_retries: int = 2  # was hardcoded in v7/scheduler.py
