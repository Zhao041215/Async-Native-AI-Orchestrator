"""V8 Architecture Design Phase — concurrent sub-phases via asyncio.gather.

Bug 1 fix: _cache_result now reads run as a Pydantic object and accesses
run.metadata.cache (dict attribute) instead of calling run.get("metadata")
which fails on Pydantic v2 models.
"""
...
CONTRACTS_PROMPT = "You are architecture_contracts_agent. Return strict JSON only. Define module_boundaries (list of {module, responsibility, public_api, depends_on}), integration_contracts (list of {from, to, contract}), and implementation_notes. Keep compact and project-specific."
...
class ArchitecturePhase:
    ...
    async def execute(
        ...
        *,
        ...
        # run may be a Pydantic Run model or a plain dict (e.g., in tests)
        metadata = self._get_metadata(run)
        scale_profile = metadata.get("scale_profile") or {}
        budget = self._build_budget(scale_profile)

        if metadata.get("architecture") and run.get("checkpoint") == "architecture_completed":
            return {"status": "completed", "project_layout": metadata.get("project_layout", {})}

        # Bug 1 fix: read cache from run.metadata.cache (Pydantic attribute path)
        cache = self._get_cache(run)

        seed = cache.get(_SEED_KEY) or metadata.get(_SEED_KEY) or await self._call_ai(
            project, run, metadata, "architecture_seed", SEED_PROMPT, budget, tenant_id, job, heartbeat,
        )
        if not seed:
        ...
        surface_task = self._cached_or_call(
            cache.get(_SURFACE_KEY) or metadata.get(_SURFACE_KEY),
            project, run, metadata, "architecture_surface", SURFACE_PROMPT, budget, tenant_id, job, heartbeat,
        )
        layout_task = self._cached_or_call(
            cache.get(_LAYOUT_KEY) or metadata.get(_LAYOUT_KEY),
            project, run, metadata, "architecture_layout", LAYOUT_PROMPT, layout_budget, tenant_id, job, heartbeat,
        )
        ...
        if isinstance(layout, Exception) or not layout:
            log.warning("layout_failed_using_default", run_id=run["id"],
                        error=str(layout) if layout else "None")
        ...
        contracts = (
            cache.get(_CONTRACTS_KEY) or metadata.get(_CONTRACTS_KEY)
            or await self._call_ai(
                project, run, metadata, "architecture_contracts", CONTRACTS_PROMPT,
                budget, tenant_id, job, heartbeat,
            )
        )
        if not contracts:
        ...
        await self._artifacts.write(
            ...
            "architecture", "architecture-design.json", design,
        )
        await self._artifacts.write(
            project["id"], run["id"], job["id"],
            "project_layout", "project-layout.json", layout_normalized,
        )
        ...
        return {
            "status": "ok",
            "architecture": design,
            "project_layout": layout_normalized,
    ...
    async def _cached_or_call(
        ...
        budget: AITaskBudget, tenant_id: str, job: dict[str, Any], heartbeat: Any | None,
    ) -> dict[str, Any] | None:
        if cached:
            return cached
    ...
    async def _cache_result(self, run_id: str, key: str, value: dict[str, Any]) -> None:
        ...
        try:
            ...
            if run is None:
                return
            # Pydantic Run model — access .metadata attribute directly
    ...
    async def _call_ai(
        ...
        result = await self._scheduler.call(
            ...
            user_payload={
                ...
                "seed": metadata.get(_SEED_KEY) or self._get_cache(run).get(_SEED_KEY, {}),
            },
        ...
        await self._artifacts.write(
            project["id"], run["id"], job["id"], task_kind, f"{task_kind}.json", output,
        )
        return output
    ...
    @staticmethod
    def _merge_design(
        seed: dict[str, Any], surface: dict[str, Any],
        layout: dict[str, Any], contracts: dict[str, Any],
    ) -> dict[str, Any]:
        return {
    ...
    def _normalize_layout(design: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "delivery_root": layout.get("delivery_root", "delivery"),
    ...
    def _default_layout(seed: dict[str, Any], surface: dict[str, Any]) -> dict[str, Any]:
        ...
        if "react" in tech_names or "vue" in tech_names or "angular" in tech_names:
            return {
                "project_layout": {
                    "source_root": "src", "delivery_root": "dist",
