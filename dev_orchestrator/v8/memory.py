"""V8 Layered Mission Memory — provides structured context for agents.

Bug 3 fix: change_memory now correctly reads changed_files from the
"agent_output" key (where ImplementationPhase actually stores it),
with fallback to the legacy "payload" key.
"""
from __future__ import annotations

from typing import Any

from dev_orchestrator.v8.models import sha256_bytes, stable_json
...
def build_layered_memory(
    *,
    ...
    for artifact in (change_artifacts or [])[-80:]:
        # Bug 3 fix: ImplementationPhase stores data under "agent_output", not "payload"
        agent_output = artifact.get("agent_output") or {}
        payload = artifact.get("payload") or {}
        changed_files = (
            agent_output.get("changed_files")
            or payload.get("changed_files")
            or []
        )
        transaction_id = (
            agent_output.get("transaction_id")
            or payload.get("transaction_id")
            or ""
        )
        ok = agent_output.get("ok") if "ok" in agent_output else payload.get("ok", True)
        changes.append({
