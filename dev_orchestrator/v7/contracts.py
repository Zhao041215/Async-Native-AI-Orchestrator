"""V7 Agent Contract Models - strict Pydantic validation for every agent task payload."""
...
from dev_orchestrator.v7.models import StrictModel
...
class IntegrationContract(StrictModel):
    source_package: str
    target_package: str
    interface_type: Literal["api", "event", "shared_type", "database"]
    specification: str
...
def validate_agent_contract(
    ...
    """Validate *payload* against the contract registered for *task_kind*.

    Returns ``(parsed_data, errors)``.  On success ``parsed_data`` is the
    model's ``model_dump()`` dict and ``errors`` is empty.  On failure
    ``parsed_data`` is ``None`` and ``errors`` contains human-readable
    messages.
    """
