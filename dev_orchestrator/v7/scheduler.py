"""V7 Async AI Call Scheduler -- three-level semaphore gating with circuit breaker."""
...
from dev_orchestrator.v7.resilience import ProviderCircuitBreaker, classify_error
...
class AsyncAIScheduler:
    ...
    def _estimate_chars(obj: Any) -> int:
        """Rough char estimate without full JSON serialization."""
    ...
    def _truncate_payload(payload: dict[str, Any], max_chars: int) -> dict[str, Any]:
        """Intelligently truncate a payload to fit within max_chars.

        Strategy: walk the dict tree, truncate the largest string fields first,
        then progressively reduce smaller fields until under budget.
        """
        serialized = stable_json(payload)
        if len(serialized) <= max_chars:
            return payload

        # Collect all string leaf paths with their lengths
        ...
        _walk(payload, [])
        # Sort by length descending -- truncate largest strings first
        ...
        for path, length in paths:
            ...
            # Truncate proportionally: keep 60% of remaining budget headroom
        ...
        *,
        ...
        for attempt in range(max_retries + 1):
            ...
            if result.ok or not result.retryable or attempt >= max_retries:
                return result
            # Don't retry circuit_open — let the breaker recover naturally
    ...
    async def _call_once(
        ...
        *,
        ...
        # Enforce context budget -- truncate oversized payloads before LLM call
        ...
        # Acquire all three semaphore levels -- ALL must be held
        try:
