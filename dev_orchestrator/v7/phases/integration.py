"""V7 Integration & Code Review Phase."""
...
class IntegrationPhase:
    ...
    async def execute(self, job: dict[str, Any], run: dict[str, Any], project: dict[str, Any], *, tenant_id: str, heartbeat: Any | None = None) -> dict[str, Any]:
        ...
        result = await self._scheduler.call(
            ...
            user_payload={"project": {"name": project.get("name", "")}, "requirements": metadata.get("requirements_analysis", {}), "architecture": metadata.get("architecture_design", {}), "package_summaries": [{"package_key": p.get("package_key", ""), "role": p.get("role", "")} for p in completed]},
    ...
    def next_job(self, run: dict[str, Any], result: dict[str, Any]) -> dict[str, Any] | None:
        if result.get("status") != "ok":
            return None
        return {"job_type": "quality", "role": "qa", "run_id": run["id"], "resume_key": f"run:{run['id']}:quality", "payload": {}}

    @staticmethod
    def _build_budget(sp: dict[str, Any]) -> AITaskBudget:
        return AITaskBudget(task_kind="integration", max_input_chars=int(sp.get("context_budget_chars", 36000)), max_output_tokens=8000, timeout_seconds=120, reasoning_effort="high", retry_attempts=int(sp.get("ai_retry_attempts", 3)))
