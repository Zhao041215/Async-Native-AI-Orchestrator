...
class QualityPhase:
    ...
    async def execute(self, job: dict[str, Any], run: dict[str, Any], project: dict[str, Any], *, tenant_id: str, heartbeat: Any | None = None) -> dict[str, Any]:
        ...
        # If no packages were generated (e.g., fallback planning), return OK immediately
        packages = await self._store.list_work_packages(run["id"])
        has_packages = len(packages) > 0
        if not has_packages:
            report = {"ok": True, "status": "GO", "gates": [], "critical_failures": 0, "major_failures": 0, "total_gates": 0, "elapsed_ms": 0, "note": "No packages to evaluate — observation-only pass"}
            await self._artifacts.write(project["id"], run["id"], job["id"], "quality_report", "quality-report.json", report)
        ...
        # AI quality gate is informational only — never blocks the pipeline.
        # Only deterministic gates (scale_profile, mission_state, dag, ai_execution) can block.
        ...
        report = {"ok": ok, "status": "GO" if ok else "NO_GO", "gates": results, "critical_failures": critical, "major_failures": major, "total_gates": len(results), "elapsed_ms": elapsed_ms}
        await self._artifacts.write(project["id"], run["id"], job["id"], "quality_report", "quality-report.json", report)
    ...
    def next_job(self, run: dict[str, Any], result: dict[str, Any]) -> dict[str, Any] | None:
        if result.get("status") != "ok":
            return None
        return {"job_type": "release_notes", "role": "release", "run_id": run["id"], "resume_key": f"run:{run['id']}:release_notes", "payload": {}}

    async def _ai_gate(self, run: dict[str, Any], project: dict[str, Any], metadata: dict[str, Any], tenant_id: str, job: dict[str, Any], heartbeat: Any | None) -> dict[str, Any] | None:
        sp = metadata.get("scale_profile") or {}
        budget = AITaskBudget(task_kind="quality", max_input_chars=int(sp.get("context_budget_chars", 36000)), max_output_tokens=4000, timeout_seconds=120, reasoning_effort="high", retry_attempts=2)
        result = await self._scheduler.call(run_id=run["id"], role="qa", job_id=job["id"], task_kind="quality", system_prompt="Return strict JSON with ok, summary, gates (list of {name, ok, severity, details}).", user_payload={"project": {"name": project.get("name", "")}}, budget=budget, store=self._store, tenant_id=tenant_id, heartbeat_callback=heartbeat)
        if not result.ok:
            return {"name": "ai_quality_gate", "ok": True, "severity": "info", "details": {"error": result.error, "note": "AI gate is informational only"}}
    ...
    async def _gate_mission_state(metadata: dict[str, Any]) -> dict[str, Any]:
        ok = bool(metadata.get("requirements") or metadata.get("requirements_analysis")) and bool(metadata.get("architecture") or metadata.get("architecture_design"))
    ...
    async def _gate_dag(metadata: dict[str, Any], observation_only: bool = False) -> dict[str, Any]:
        ...
        return {"name": "package_dag_acyclic_gate", "ok": ok or observation_only, "severity": severity, "details": {"observation_only": observation_only}}
    ...
    async def _gate_ai_execution(self, run: dict[str, Any], observation_only: bool = False) -> dict[str, Any]:
        ...
        ok = len(patch_sets) > 0
        severity = "info" if observation_only else "critical"
        return {"name": "ai_live_execution_gate", "ok": ok or observation_only, "severity": severity, "details": {"count": len(patch_sets), "observation_only": observation_only}}
